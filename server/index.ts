...
// ── Simple global rate limiter: 5 requests per 60 s ──────────────────────────
  ...
  app.post("/api/ai-projection", async (req, res) => {
    ...
    try {
      const {
        ...
        forecastStartMonth: string; // YYYY-MM
        forecastMonths: number; // how many months to forecast
      };

      // Use up to the past 2 years of historical data; fall back to all data if less is available.
      const now = new Date();
      ...
      // Split transactions: past (used for historical patterns) vs future one-time events
      ...
      // Future one-time transactions (explicit events the AI should apply on the right month)
      ...
      function summariseRecurring(items: RecurringItem[]) {
        return items.map((r) => ({
          description: r.description,
          frequency: r.frequency,
          amountPerOccurrence: parseFloat(r.amount),
          monthlyEquivalent: +monthlyEquivalent(r.amount, r.frequency).toFixed(
            2,
          ),
      ...
      const recurringExpSummary = summariseRecurring(
        recurringExpenses as RecurringItem[],
      );
      const recurringIncSummary = summariseRecurring(
        recurringIncomes as RecurringItem[],
      );
      ...
      // Net monthly from recurring alone (typical month, all items active)
      const baseRecurringNet = totalRecurringInc - totalRecurringExp;

      // ── Estimate baseline variable spending from past data ─────────────────
      // Group past expenses by YYYY-MM to get monthly totals
      ...
      // Group future one-time events by YYYY-MM for easy AI reference
      const futureExpByMonth: Record<
        string,
      ...
      // Fallback variable expense base (used when Gemini fails or returns too few)
      const varExpBase =
        avgMonthlyVarExp ??
        Math.max(200, Math.min(500, totalRecurringInc * 0.15));

      // Deterministic fallback variable expense for month index i
      function fallbackVarExp(i: number): number {
        const ym = forecastMonthLabels[i]!;
        const monthIdx = parseInt(ym.slice(5, 7), 10) - 1; // 0-based
        const inflation = Math.pow(1.003, i); // 0.3%/month
        const seasonal = SEASONAL[monthIdx] ?? 1.0;
        // Small pseudo-random noise seeded on month index (deterministic)
        const noise = 1 + (((i * 7 + 3) % 17) - 8) / 80; // ±10%
...
- Average total past expenses: €${avgMonthlyVarExp}/month (includes groceries, dining, clothing, fuel, etc.)
- Individual past transactions:\n${pastTxSummary}`
      ...
      const varExpBudgetHint =
        typicalSurplusAfterRecurring > 0
          ? `The person has ~€${typicalSurplusAfterRecurring.toFixed(0)}/month left after all recurring charges. Variable day-to-day expenses must realistically fit within this budget.`
          : `After recurring charges the person is tight on budget (net: €${typicalSurplusAfterRecurring.toFixed(0)}/month). Estimate conservatively.`;
